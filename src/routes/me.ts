...
export function toMeRow(row: FacadeRow, session: PortalSession): MeLedgerRow {
  ...
  const isTx = !!ref && /^[0-9a-fA-F]{64}$/.test(ref)
  const mode: MeLedgerRow['mode'] = row.channel_cursor_after ? 'session' : isTx ? 'x402-exact' : 'mpp-charge'
...
export async function handleMeLedger(request: Request, env: Env): Promise<Response> {
  ...
  // Cursor is the created_at (ms) of the last row on the previous page.
  const cursorRaw = url.searchParams.get('cursor')
  const before = cursorRaw ? Number(cursorRaw) : null
  if (cursorRaw && (!Number.isFinite(before) || before! <= 0)) {
    return json(400, { error: 'cursor must be a positive integer timestamp.' })
  }
  ...
  `).bind(session.payer, before ?? Number.MAX_SAFE_INTEGER, limit + 1).all<FacadeRow>()

  const all = result.results ?? []
  const page = all.slice(0, limit)
  const rows = page.map((r) => toMeRow(r, session))
  const next_cursor = all.length > limit && page.length > 0 ? String(page[page.length - 1].created_at) : null
