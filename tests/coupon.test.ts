...
describe('POST /coupon/redeem', () => {
  ...
  it('pay-invoice failure parks in manual_review (reported as processing), never auto-retries payment', async () => {
    ...
    // Runbook step 1: while the coupon is still in manual_review, a human
    // verifies Coinbase is NOT settled and clears the gate (recorded), naming
    // the exact holder.
    const holder = (await readCoinbaseExecGate(env, 'pl_test123'))!.holder
    const clearReq = (expectedHolder: string) =>
    ...
    const clr = await handleCoinbaseExecGateClear(clearReq(holder), env)
    expect(clr.status).toBe(200)
    expect(((await clr.json()) as any).changed).toBe(true)
    // Step 2: release the coupon → issued; the redeem works again.
...
describe('coupon redemption: Coinbase exec gate', () => {
  ...
  it('released coupon whose pay request was sent stays blocked; clear refused once the coupon left manual_review', async () => {
    const { env, calls, fetchMock } = makeEnv({ payStatus: 502, payBody: { error: 'boom' } })
    globalThis.fetch = fetchMock
    const code = await issueCoupon(env)
    await handleRedeemCoupon(redeemReq(code), env)
    expect(calls.pay).toBe(1)
    const holder = (await readCoinbaseExecGate(env, 'pl_test123'))!.holder
    await handleResolveCoupon(resolveReq({ code, action: 'release', reason: 'x' }), env)
    // Gate still held → re-redeem refused, pays nothing, coupon usable.
    const blocked = await handleRedeemCoupon(redeemReq(code), env)
    expect(blocked.status).toBe(409)
    expect(((await blocked.json()) as any).error).toBe('LINK_CLAIMED')
    expect(calls.pay).toBe(1)
    // The coupon is no longer in manual_review for that attempt → refuse.
    const clr = await handleCoinbaseExecGateClear(
      new Request('https://router.test/admin/coinbase-exec-gate/clear', {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'x-admin-secret': 'test-pay-secret' },
        body: JSON.stringify({ plId: 'pl_test123', evidence: 'operator wants a re-pay for testing', clearedBy: 'ops-test', expectedHolder: holder }),
      }),
      env,
    )
    expect(clr.status).toBe(409)
    expect((await readCoinbaseExecGate(env, 'pl_test123'))?.holder).toBe(holder)
  })

  it('admin clear refuses a webhook holder with no parked fulfillment record', async () => {
    const { env, fetchMock } = makeEnv()
    ...
    const clr = await handleCoinbaseExecGateClear(
      new Request('https://router.test/admin/coinbase-exec-gate/clear', {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'x-admin-secret': 'test-pay-secret' },
        body: JSON.stringify({ plId: 'pl_test123', evidence: 'operator wants a re-pay for testing', clearedBy: 'ops-test', expectedHolder: 'evt-webhook-1' }),
      }),
      env,
    )
    expect(clr.status).toBe(409)
