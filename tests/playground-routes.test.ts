...
describe('POST /v1/playground/chat', () => {
  it('rejects a model that is not on the allow-list, before reserving anything', async () => {
    ...
    expect(body.allowed_models).toContain('openai/gpt-oss-20b')
  ...
  it('rejects malformed message arrays', async () => {
    ...
    for (const messages of cases) {
      const r = await handlePlaygroundChat(
        post('/v1/playground/chat', { model: 'openai/gpt-oss-20b', messages }, token),
        env,
      )
      expect(r.status).toBe(400)
  ...
  it('402s with the remaining balance when the session cannot cover the call', async () => {
    ...
    const r = await handlePlaygroundChat(
      post(
        '/v1/playground/chat',
        { model: 'openai/gpt-oss-20b', messages: [{ role: 'user', content: 'hi' }] },
        emptyToken,
      ),
      empty,
    )
  ...
  it('rejects a caller-supplied call_id that is not an opaque id', async () => {
    ...
    const r = await handlePlaygroundChat(
      post(
        '/v1/playground/chat',
        { call_id: '../../etc/passwd', model: 'openai/gpt-oss-20b', messages: [{ role: 'user', content: 'hi' }] },
        token,
      ),
      env,
    )
...
describe('dispatch-marker failure aborts before any paid upstream call', () => {
  it('releases the reservation and returns 503 when the DO dispatch write fails', async () => {
    ...
    const r = await handlePlaygroundChat(
      post('/v1/playground/chat', { model: 'openai/gpt-oss-20b', call_id: 'dispatch-fail', messages: [{ role: 'user', content: 'hi' }] }, token),
      env,
    )
    expect(r.status).toBe(503)
